/* ==================================================================
 * SnsAuthorizationBuilder.java - 13/08/2021 12:52:37 PM
 * 
...
package net.solarnetwork.node.setup.stomp.client.util;

import static net.solarnetwork.node.setup.stomp.client.util.AuthorizationUtils.AUTHORIZATION_DATE_FORMATTER;
import static net.solarnetwork.node.setup.stomp.client.util.AuthorizationUtils.AUTHORIZATION_TIMESTAMP_FORMATTER;
import static net.solarnetwork.node.setup.stomp.client.util.AuthorizationUtils.computeHmacSha256;
...
/**
 * Builder for {@code Authorization} header values using the SolarNode Setup (SNS) authentication
 * scheme.
 * 
 * <p>
 * This helper is designed with different communication protocols in mind, such as HTTP and STOMP,
 * so the terms used are generic enough to apply in different contexts.
 * </p>
 * 
 * @author matt
 * @version 1.0
 */
public final class SnsAuthorizationBuilder {
   ...
   * 
   * <p>
   * The builder will be initialized and then {@link #reset()} will be called so default values are
   * configured.
   * </p>
   * 
   * @param identifier
   ...
   * 
   * <p>
   * This will also set the {@literal date} header with the date's formatted value.
   * </p>
   * 
   * @param date
   *          the date to use, or {@literal null} for the current system time via
   ...
   * 
   * <p>
   * The meaning of the verb depends on the communication protocol being used, such as a HTTP method
   * or STOMP command.
   * </p>
   * 
   * @param verb
   ...
   * 
   * <p>
   * This is a shortcut for calling {@code #header(String, String)} with a {@literal host} key.
   * </p>
   * 
   * @param host
   *          the host value
   ...
   * 
   * @param digest
   *          the digest value to use or {@literal null} for none; if provided, the array must have
   *          a length of {@literal 32} and will be copied
   * @return this builder
   */
   ...
   * 
   * <p>
   * Header values are <b>replaced</b> if a given {@code headerName} is passed more than once.
   * </p>
   * 
   * @param headerName
   ...
   * @return the header values, or {@literal null} if the header does not exist
   */
   ...
   * 
   * <p>
   * Signing keys are derived from the a secret value and valid for 7 days, so this method can be
   * used to compute a signing key so that {@link #build()} can be called later. The signing date
   * will be set to whatever date is currently configured via {@link #date(Instant)}, which defaults
   * to the current time for newly created builder instances.
   * </p>
   * 
   * @param secret
   ...
   * 
   * <p>
   * Use this method if a signing key has been computed externally. The effect is the same as in
   * {@link #saveSigningKey(String)} in that the {@link #build()} method can then be called to
   * compute the authorization value using this key.
   * </p>
   * 
   * @param key
   ...
   * 
   * @return the computed or saved signing key encoded as hex, or {@literal null} if none computed
   *         or saved yet
   */
   ...
   * 
   * <p>
   * A signing key is derived from {@code key} and {@code date} using the following algorithm:
   * </p>
   * 
   * <pre>
   * <code>
   ...
   * 
   * <p>
   * The {@code HmacSha256(key, message)} function computes a HMAC+SHA256 digest value. The
   * {@literal YYYYMMDD} value is {@code date} formatted in the UTC time zone.
   * </p>
   * 
   * @param date
   *          the signing date
   ...
   * @param signatureData
   *          the signature data, e.g. {@link #computeSignatureData(Instant, String)}
   ...
   *          the canonical request message, i.e. from {@link #computeCanonicalRequestMessage()}
  ...
  /**
   * Compute an {@literal Authorization} header value from the configured properties on the builder,
   * using a signing key created from a previous call to {@link #saveSigningKey(String)} or
   * {@link #signingKey(byte[])}.
   * 
   * <p>
   ...
   * 
   * <p>
   * Where {@code identifier} is the identifier passed to the constructor, {@code headerList} is a
   * semicolon-delimited list of {@link #sortedHeaderNames()}, {@code signingKey} is the signing key
   * set via {@link #signingKey(byte[])} or computed via {@link #saveSigningKey(String)}, and
   * {@code signatureData} is the computed signature data via
   ...
   * 
   * @param secret
   *          the secret to sign the digest with; will use the configured date to compute the
   *          singing key
   * @return the header value
   * @throws SecurityException
   *           if any error occurs computing the header value
  ...
  /**
   * Compute a signature value from the configured properties on the builder, using a signing key
   * created from a previous call to {@link #saveSigningKey(String)} or {@link #signingKey(byte[])}.
   * 
   * <p>
   * <b>Note</b> this method returns just the signature value, not a complete
   * {@literal Authorization} header value. Use the {@link #build()} method to generate a complete
   * header value.
   * </p>
   * 
   * <p>
   ...
   * 
   * <p>
   * Where {@code signingKey} is the signing key set via {@link #signingKey(byte[])} or computed via
   * {@link #saveSigningKey(String)}, and {@code signatureData} is the computed signature data via
   * {@link #computeSignatureData(Instant, String)}.
   * </p>
   * 
  ...
  /**
   * Compute a signature value from the configured properties on the builder, using the provided
   * secret.
   * 
   * <p>
   * <b>Note</b> this method returns just the signature value, not a complete
   * {@literal Authorization} header value. Use the {@link #build(String)} method to generate a
